...
const AddProperty = () => {
  ...
  const fetchUsersInfo = async (email) => {
    const response = await axiosSecure.get(`/users/email/${email}`);
    return response.data;
  };
  // React Query to fetch userInfo
  const {
    data: userInfo,
    ...
    queryFn: () => fetchUsersInfo(user?.email),
  });

  // Use useMutation for the POST request
  const mutation = useMutation({
    mutationFn: async (data) => {
      const response = await axiosSecure.post("/properties", data);
      return response;
    },
    onSuccess: () => {
      showAlert(
  ...
  if (errorUserInfo)
    return <div>Error fetching properties: {errorUserInfo.message}</div>;

  const handleSubmit = async (e) => {
    e.preventDefault();
    setErrors({});

    if (userInfo[0]?.fraud === "yes") {
      showAlert(
        "error",
        "Access Denied",
        "You cannot add a property due to fraud status."
      );
      return;
    }
