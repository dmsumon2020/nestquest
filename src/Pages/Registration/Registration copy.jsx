...
const Registration = () => {
  ...
  const imgBBUpload = useImgBBUpload();
  ...
  const handleCreateUser = async (event) => {
    ...
    try {
      ...
      // Use the ImgBB mutation
      imgBBUpload.mutate(photoFile, {
        onSuccess: async (imageUrl) => {
          // Update Firebase user details
          const updatedUser = await updateUsernameAndPhotoUrl(
            username,
            imageUrl
          );
          setUser(updatedUser);
          ...
          // Use the registration mutation
          registerMutation.mutate(dataForDb);
        },
        onError: (error) => {
          setErrors((prevError) => ({
            ...prevError,
            photoUrl: error.message || "Image upload failed",
          }));
        },
      });
    } catch (firebaseError) {
